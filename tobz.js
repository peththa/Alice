    ...
    try {
        ...
        // FUNCTION
                ...
                if(body === '#mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin Alice!', id)
                ...
                if (body === '#unbanchat') {
                    if (!isOwner) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner Alice!', id)
        ...
        case '#banchat':
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Alice!', id)
...
${desc}`)
        ...
        case '#owner':
        case '#creator':
            tobz.sendContact(chatId, `6281338888294@c.us`)
        ...
        case '#simi':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin Alice!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
                simi_.push(chat.id)
...
🌐️ *URL*: ${url}`
        ...
        case '#video':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            tobz.reply(from, 'PREMIUM COMMAND, HUBUNGI : wa.me/6281338888294', id)
            break
        case '#getvideo':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            tobz.reply(from, 'PREMIUM COMMAND, HUBUNGI : wa.me/6281338888294', id)
...
➸ *Wilayah:* +${region_name}`
        ...
        // ADMIN & OWNER
        case '#bc':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Owner Alice!', id)
            let msg = body.slice(4)
            const chatz = await tobz.getAllChatIds()
            for (let ids of chatz) {
                var cvk = await tobz.getChatById(ids)
                if (!cvk.isReadOnly) await tobz.sendText(ids, `[ Alice BROADCAST ]\n\n${msg}`)
            }
        ...
        case '#omentionall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner Alice', id)
        ...
        case '#okickall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Admin Alice', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case '#oadd':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Alice', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case '#oleave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Alice', id)
            await tobz.sendText(from,'Alice DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case '#leave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
        ...
        case '#getses':
            ...
            break
        case '#aliceadmin':
            let admn = `This is list of Alice Admin\nTotal : ${adminNumber.length}\n`
        ...
        case '#addadmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Alice!', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin Alice!', id)
                }
            break
        case '#deladmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Alice!', id)
                ...
                fs.writeFileSync('./lib/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Success Menghapus Admin Alice!', id)
            break
        case '#block':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Alice!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case '#unblock':
            ...
            break
        case '#ban':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin Alice!', id)
                for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case '#help':
            ...
            break
        case '#alicegroup':
            tobz.reply(from, `Link Group Alice : https://chat.whatsapp.com/By906EiJBGBCZGURDadOat\nJangan Lupa Join Ya Kak ${pushname}`, id)
            break
        case '#groupmenu':
        ...
        case '#iklan':
            ...
            break
        case '#adminmenu':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Alice', id)
        ...
        case '#snk':
            ...
            break
        default:
            if (!isGroupMsg) return tobz.reply(from, 'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup Alice, Link Ada Di Bio atau Bisa Mengetik #Alicegroup!\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *#iklan*', id)
            if (command.startsWith('#')) {
